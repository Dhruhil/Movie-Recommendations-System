...
def process_batch(batch):
    ...
    query = f"""
    SELECT DISTINCT ?targetMovie ?actor ?actorLabel WHERE {{
        VALUES ?targetMovie {{ {movie_filter} }}
        ?targetMovie wdt:P161 ?actor .
    ...
    while attempts < retry_attempts:
        try:
            ...
            for result in results:
                ...
                actor_uri = URIRef(result["actor"]["value"])
                actor_name = Literal(result["actorLabel"]["value"], lang="en") if "actorLabel" in result else Literal("Unknown", lang="en")

                # Add actor triples to the graph
                g.add((actor_uri, RDF.type, entities.Q5))  # Instance of human
                g.add((actor_uri, RDFS.label, actor_name))
                g.add((movie_uri, properties.P161, actor_uri))  # Actor relationship
...
output_file = "movies_with_actors.ttl"
