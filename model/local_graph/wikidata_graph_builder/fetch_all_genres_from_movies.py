...
def process_batch(batch):
    ...
    query = f"""
    SELECT DISTINCT ?targetMovie ?genre ?genreLabel WHERE {{
        VALUES ?targetMovie {{ {movie_filter} }}
        ?targetMovie wdt:P136 ?genre .
    ...
    while attempts < retry_attempts:
        try:
            ...
            for result in results:
                ...
                genre_name = Literal(result["genreLabel"]["value"], lang="en") if "genreLabel" in result else Literal("Unknown", lang="en")

                # Add genre triples to the graph
                #g.add((genre_uri, RDF.type, entities.Q5))  # Instance of human
                g.add((genre_uri, RDFS.label, genre_name))
                g.add((movie_uri, properties.P136, genre_uri))  #genre relationship
...
output_file = "movies_with_genres.ttl"
